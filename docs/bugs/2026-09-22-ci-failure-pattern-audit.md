...
| Trace descriptor lifetimes | fs_usage omits shm_open and suppresses dup/dup2 in the selected mode; the parser also ignores close variants. | Native trace contains 23 unattributed ftruncate calls. A later descriptor snapshot alone cannot establish earlier write targets. Task-scoped DTrace authorized; coverage probe awaits Terminal authentication. See the dated fs_usage ownership report. |
