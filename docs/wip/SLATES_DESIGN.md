...
> **Status (2026-09-22, council seats follow incumbency and liveness, not id order).** The council's voters were the members with the lowest ids up to `2f + 1`, whatever their state. In the KIND lane a killed owner's replacement authenticated under the same certificate with a fresh id and was admitted before its predecessor's death had held for the confirmation window; with four members the leader moved the voter set to the three lowest ids, so the dead predecessor kept a seat and the live leader lost its own. The replacement then won the election, and it cannot observe its own predecessor's death (it has no probe path to its own certificate), so it never retired it: the dead id stayed in the committed configuration, no takeover was assigned, and neither survivor served the volume (CI run 35615113353; reproduced one failure in eight on a CPU-pinned local cluster, the committed voter set read back with `slates recovery-plan region`). Now `council_voters(sitting, members, alive, quorum)` keeps every sitting voter that is still a member (a seat is freed only by a member leaving the configuration, never by a suspicion), fills a free seat only from members the leader holds authenticated-alive, and uses the id only as the tiebreak; the leader alone computes the target, since every node learns the voter set from the Raft log. No Raft rule changed. Regression: `config_group::tests::a_replacement_admitted_before_its_predecessor_retires_takes_no_live_voters_seat` (failed before, ids in the KIND order) and `a_free_seat_waits_for_a_member_the_leader_holds_alive`. Owed: the replacement's blind spot to its own predecessor after two overlapping faults (needs each member's certificate anchor in the configuration, or follower death evidence), and the same incumbency rule for root-group representatives. Record: `docs/bugs/2026-09-22-council-seats-follow-id-order-not-liveness.md`.
...
### A-29 — Council seats follow incumbency and liveness, not id order (2026-09-22)

The council's voter set was a pure function of its members: the lowest ids up to `2f + 1`. A member id
is derived from a certificate and a per-boot nonce, so id order has no meaning for voting, and the rule
let a replacement admitted beside its unretired predecessor take a live voter's seat while the dead
predecessor kept its own. Replace it: a sitting voter keeps its seat while it is still a member; a seat
freed by a member leaving the configuration goes only to a member the leader holds authenticated-alive;
the id is only the tiebreak. The leader alone computes the target; every node learns the voter set from
the Raft log's joint-consensus entries, as before.

Applied in the same change to: §4.8 (status); `slates-cluster` `config_group` (`council_voters`,
