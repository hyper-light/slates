...
> deriver (a journal of content, create and unlink composed into one ops document — create-unlink
> cancellation, base-file replacement, the post-state laid out in sorted-path order, invalid
> operations typed refusals — with a whole-filesystem reconstruction oracle). All on every host.
> The deriver's rename composition (and directories, links, metadata), the checkpoint folding of
> the canonical deltas, the splice by extent surgery, the green chain and the fenced ledger
> register, and holder recomputation are the rest of Phase 6.
