...
> per path (directory creation, mode and symlink, with type, metadata and differing-target
> conflicts). Directory removal, rename, link and xattr merges through the engine (the last needing
> the engine to consume the whole ops document), per-range identity, the checkpoint folding of the
> canonical deltas, and the green chain are the rest of Phase 6; the fleet register, mirror and
> reconfiguration protocols are now simulated (§4.8 status, GAPS §8h).
