...
> coordinates, overlaps handed to the verdict, with a provenance oracle), and the increment
> assembler (a whole-volume content journal composed into one ops document, the post-state laid
> out in sorted-path order, with a reconstruction oracle). All on every host. The deriver's
> namespace composition (create, unlink, rename, directories, links, metadata), the checkpoint
