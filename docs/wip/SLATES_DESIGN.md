...
> per path (directory creation, mode, symlink and file rename, with type, metadata, differing-target
> and rename/rename conflicts; a rename captures the source content so an intervening edit follows
> the move, and removes apply before sets so a chained rename rotates correctly). Directory removal,
