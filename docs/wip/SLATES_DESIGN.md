...
Applied in the same change to: §4.8 (Membership, new "Slow versus stuck"), the `slates-cluster` crate, the `slates-db` register (phase-one promotion), GAPS §1 (Registers/configuration status).
