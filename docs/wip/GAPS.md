...
the one reader. Still owed: the **Windows** completion fd (its
async loop needs a socket, D-10 "asyncio on Windows needs a socket"; a client-local loopback
socketpair fed by the same bridge, build-and-lint only from this host); and the **Node `uv_poll`**
binding, the same shape over libuv (`begin`/`spin_reply`/`poll_reply`) behind its thin sync facade
(R6, D-19); and the rest of the async verb set past create/snapshot/status. The Rust client parks
on the word and needs none. The Windows named Event per client (Phase
