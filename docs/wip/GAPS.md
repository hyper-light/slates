...
status lifecycle and eight `Promise.all`-ed creates, each a distinct id. Still owed: the **Windows**
completion fd (its async loop needs a socket, D-10 "asyncio on Windows needs a socket"; a
client-local loopback socketpair fed by the same bridge, build-and-lint only from this host); and
the rest of the async verb set past create/snapshot/status, in both SDKs. The Rust client parks on
the word and needs none. The Windows named Event per client (Phase
