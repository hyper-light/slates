...
tree built and submitted). Still owed: only the **Windows** completion fd (its async loop needs a
socket, D-10 "asyncio on Windows needs a socket"; a client-local loopback socketpair fed by the same
bridge, build-and-lint only from this host — and itself gated on the Windows named-Event wake), and
the async `land` (R10-gated on a human grant). The Rust client parks on the word and needs none. The Windows named Event per client (Phase
