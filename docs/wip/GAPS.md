...
| Bridges (4.6) | Linux FUSE codec/dispatch/transport/launcher; the NFSv3 loopback bridge (`bridge-nfs`, the macOS fallback + oracle). The **macOS FSKit bridge** now has its Rust half started (`crates/bridge-fskit`, A-1/D-O9): the shim wire codec for the identity-addressed read path (Lookup/GetAttr/Read) dispatched onto the one `Bridge` seam, tested by use against a real `VolumeBridge` (round-trip, hostile-input, and a create→lookup→getattr→read walk with a NotFound error reply) — codec-first, no socket/mount/Swift, exactly as the NFS codec began. Owed: the Swift `FSVolume` shim + `Slates.app` bundle + FSKit entitlement + app-group ring + mount spike (Phase 4, the collaboration); the handle path (opendir/readdir/open/release), write and namespace operations with a golden reply test; and the WinFsp bridge. | GAP-A9-3–5; AC-3.10–3.12, AC-4.11–4.12 |
