...
Owed (the rest of Phase 6): the deriver's rename composition (mapping later operations, and, for a new file renamed over a base file, the write-and-rename content replacement) plus directories, hard links, symlinks and metadata; the checkpoint folding above; the splice by extent surgery, the green
