...
Owed (the rest of Phase 6): directories, hard links, symlinks and metadata (mode, xattrs) in the deriver, and the rare reused-base-path rename combination above; the checkpoint folding above; the green
...
Gated (`crates/archive/tests/archive.rs`, 11 tests, every host): an archive round-trips (T-7.1), encoding is deterministic, the seek table finds a chunk by identity (and reports an unknown one absent), a chunk that fails its identity is refused and named (AC-7.3), a bad magic and an unknown major are refused, a flipped body byte is caught by the whole-archive hash, a short stream is refused; and three hostile proptests (T-7.4): every truncation is refused, any single byte flip is caught (never the original snapshot decoded), and arbitrary bytes never panic. Raw encoding only; the codec pass (LZ4, zstd with static contexts, dictionaries, FastCDC), the identity/dedup pass, the manifest tree's structure, the export stream through the SDKs/MCP, and restore are the rest of Phase 7 (the format reserves the encoding and dictionary fields, so an archive written then reads now). No cross-compiled C codec is pulled in: the raw container needs only BLAKE3.
