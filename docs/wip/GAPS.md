...
| Bridges (4.6) | Linux FUSE codec/dispatch/transport/launcher; the NFSv3 loopback bridge (`bridge-nfs`, the macOS fallback + oracle). The **macOS FSKit bridge** now has its Rust half **complete** (`crates/bridge-fskit`, A-1/D-O9): the shim wire codec for the whole `Bridge` operation set (read/write/handles/dir-enumeration/namespace/links/refs/metadata — 20 operations) dispatched onto the one seam, a golden vector pinning the wire, 9 by-use tests against a real `VolumeBridge` (round-trip, hostile-input, a directory lifecycle, symlink→readlink, create→rename→lookup, a chmod+truncate setattr, a NotFound reply). Codec-first, exactly as the NFS codec. The Swift half now **compiles against the real framework** (macOS 15.4+ SDK): `ShimWire.swift` is the codec (a pure library, cross-checked byte-for-byte against the Rust golden vector), and `SlatesVolume.swift` is the `FSVolume` + `FSUnaryFileSystem` handler — it conforms to FSKit's real `FSVolume.Operations`/`ReadWriteOperations`/`OpenCloseOperations` and `FSUnaryFileSystemOperations`, translating every operation to a shim request; `swiftc -emit-library` builds a dylib exporting `SlatesVolume`/`SlatesItem`/`SlatesFileSystem` with `@objc` conformance thunks over the real FSKit signatures (built on macOS 26 here; the CI step skips loud on a pre-15.4 runner). Owed (the Phase 4 mount spike): the app-group ring behind the `ShimChannel` seam, the `Slates.app` bundle + FSKit entitlement + `UnaryFileSystemExtension` `@main`, and the live mount that confirms the `FSItem` lifecycle and the `SPIKE:`-marked semantics (root object id, time unit, open refcounting). The 20th op, `OP_SETATTR`, is now wired end to end: `setAttributes` carries only the fields FSKit marks valid (chmod/chown/truncate/utimes) to the bridge's `setattr` and returns the new attributes, verified over a real `VolumeBridge` (`serve_sets_attributes_through_the_bridge`) and round-tripped on the Swift side. The **WinFsp bridge** has its host-buildable half started (`crates/bridge-winfsp`): the refusal taxonomy `ntstatus(&VfsError) -> Ntstatus` (the analogue of the FUSE errno and NFS `nfsstat3` edges), the common codes mapped precisely and the rare/slates-specific ones to the generic `STATUS_UNSUCCESSFUL` pending `ntstatus.h`, tested by use. The rest is genuinely Windows-bound — unlike FSKit's shim wire (ours to define), WinFsp's `FSP_FSCTL_TRANSACT` is winfsp's header-defined protocol, so the marshalling and the `winfsp-rs` FFI need the winfsp headers on Windows (they cross-lint on the native Windows CI runner). | GAP-A9-3–5; AC-3.10–3.12, AC-4.11–4.12 |
