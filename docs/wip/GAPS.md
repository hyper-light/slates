...
The increment assembler — composing a whole-volume content journal into one ops document — landed the same day (`increment.rs`). It groups the journal by path (`FileOp`, a content op tagged with its file), composes each path with the content deriver, and lays the results out as one `OpsDoc`. The post-state layout: the paths' final contents concatenated in sorted-path order, so each path occupies a contiguous region and an op's source offset is its offset within the path's final content (what the deriver returns) plus that region's base offset; sorting the paths makes the layout, and the identity, independent of the journal's declaration order. To lay the regions out the deriver now also returns each path's final content length (`compose_content_sized`). Gated (`crates/merge/tests/increment.rs`, 4 tests, every host): two files assemble and each reconstructs, the path table holds the touched paths sorted, the identity is independent of declaration order (two interleavings of the same per-path work hash the same), and an assembler oracle over three files with random journals — every path reconstructs from its base and its net ops drawing from the global post-state, with base bytes (0-127) and added bytes (128-255) in disjoint ranges so any op on the wrong path, offset or source diverges. A path the journal never touches is absent from the document and its final equals its base.

Owed (the rest of Phase 6): the deriver's namespace composition (create/unlink with their cancellation, rename mapping later operations and, for a new file renamed over a base file, the write-and-rename content replacement, plus directories, links, symlinks and metadata); the checkpoint folding above; the splice by extent surgery, the green
