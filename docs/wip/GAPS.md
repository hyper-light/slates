...
the Event wake is exercised on a real Windows runner, not just linted. Still owed: the **Windows
completion fd** (its async loop needs a socket, D-10 "asyncio on Windows needs a socket"; a
client-local loopback socketpair whose bridge thread parks on this same Event) and the full Win32
rendezvous (Phase 4) that a client connects through. The Rust client parks on the word and needs
