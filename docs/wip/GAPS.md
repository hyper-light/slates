...
Owed (the rest of Phase 8): wiring the register into the server's put path (hedging content to `f+1` with tied requests to the rest after the measured p95, the acknowledging set recorded in the head record), the healer and probation for a lagging or returned holder, mirroring committed records to the mirror region in epoch order with an exposed lag and `await placed(mirror)`, migration of ownership on a write-intent attachment, the SWIM membership feeding the neighbourhood and host epochs, and the regional configuration consensus group (the hecate Raft dialect) that assigns epochs at takeover; the auto-seal cadence constants remain measured in Phase 8 (D-O12). The simulation stands in for all of these at the protocol level; the runtime carries the same code with a real transport.
