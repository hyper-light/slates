...
Owed (the rest of Phase 6): hard links in the deriver (they need inode reference-counting across names), a file/directory transition at one path, chmod/xattr then rename, symlink rename, and the rare reused-base-path rename combination above; the checkpoint folding above; the green
