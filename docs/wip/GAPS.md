...
Owed (the rest of Phase 6): the deriver's namespace composition (create/unlink/rename and the rest, with the create-then-unlink and rename-remap cancellations) and the multi-path assembly into a whole ops document; the checkpoint folding above; the splice by extent surgery, the green
