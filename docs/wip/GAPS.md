...
Still owed: the **Windows** completion fd (its async loop needs a socket, D-10 "asyncio on Windows
needs a socket"; a client-local loopback socketpair fed by the same bridge, build-and-lint only from
this host); and the namespace async operations (unlink/rename/mkdir/… — the same typed shape; the
volume lifecycle and the whole merge workflow are now async in both). The Rust client parks on the
word and needs none. The Windows named Event per client (Phase
