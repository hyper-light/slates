...
the same shape builders (`submitted_to_py`/`submit_outcome`, `work_dict`/`work_volume`). Still owed:
the **Windows** completion fd (its async loop needs a socket, D-10 "asyncio on Windows needs a
socket"; a client-local loopback socketpair fed by the same bridge, build-and-lint only from this
host); and the remaining async verbs (versions/changed_since/rebase and the namespace operations —
the same typed three-line shape). The Rust client parks on the word and needs none. The Windows named Event per client (Phase
