...
| Verify the displaced file after the swap | Complete fingerprint before exchange; open the actual displaced name afterward, compare identity/metadata and hash content when ctime changed or the witness was racy. The exchange itself updates ctime (M: Linux tmpfs, 2026-09-20; see the landing exchange bug record). | same verification contract | our handle on the old file, opened without write sharing, stays valid across the POSIX-semantics replace; a sharing violation from another process's handle is reported as `TargetInUse` |
