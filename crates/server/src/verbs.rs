...
fn acknowledge(state: &mut ShardState, client_id: u32, up_to: u32) -> ReplyBody {
  let now = state.clock.monotonic_ns();
  // A local client acknowledges its own completions, keyed under this node's **stable cert-anchor** —
  // the key `serve` records and looks them up by (task #22) — never the ephemeral member id, which
  // hashes the boot generation and so names a window no record was ever written to (an acknowledgement
  // keyed there released nothing, and every acknowledged request stayed answerable from its record:
...
fn retry_deferred(state: &mut ShardState) -> bool {
  ...
  for entry in deferred {
    ...
    // stable cert-anchor — the key `serve` looks a retry up by (task #22), never the ephemeral member id; a
    // cross-node forward records on the owner under its authenticated origin instead.
    let origin = state.origin_anchor.0;
