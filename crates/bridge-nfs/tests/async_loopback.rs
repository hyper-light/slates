...
//! `mount_nfs` client does over the wire. It proves the accept -> read -> dispatch -> bridge -> volume
//! path runs on the runtime and reaches the RAM-only volume, in CI on any host, with no kernel mount
//! and no privilege. The `nfs_async` example serves the same `serve_connection_async` to a real
//! `mount_nfs`.
...
const MESSAGE: &[u8] = b"hello from slates, served live over NFS on the runtime\n";
const MOUNT_PROGRAM: u32 = 100_005;
const NFS_PROGRAM: u32 = 100_003;
/// Shape: the test makes exactly one connection, so a tiny backlog suffices.
const BACKLOG: i32 = 4;

// --- The server side: a demo volume, seeded, served on the runtime. ---
...
// --- The client side: a minimal ONC RPC/NFSv3 client over the socket (as in `loopback.rs`). ---
...
/// The async server on the runtime serves a mount and a read over a real socket: the listener is bound
/// on the test thread (so the port is known before the client connects) and moved onto the shard, where
/// the non-`Send` serve loop runs; a std client mounts `/`, looks up the seeded file, and reads back its
/// exact bytes — the whole socket -> runtime -> dispatch -> bridge -> volume path.
#[test]
fn a_client_mounts_and_reads_a_file_over_the_async_server() {
  ...
  assert_eq!(
    data, MESSAGE,
    "the client read back the exact bytes the server seeded, over the async server on the runtime"
  );
